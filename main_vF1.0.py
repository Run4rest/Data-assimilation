...
# Main script for searching params with point_dim = 5 (TRIALS = 500)
...
from environment_vF01 import ODEEnv

DBFILE = '.\\log\\db_vF01.npy'
CMFILE = '.\\log\\cm_vF01.npy'
LOG = '.\\log\\log_vF01.log'
...
FIRST_START = 0   # this flag indicates 2 options:
                  # 1 - starting a new optimization
                  # 0 - continuing the previous one  
TRIALS  = 500     # this is number of points with uniform PDF to calculate a reward, but after 
                  # selecting points belonged spherical neighborhood of a maximum, real number 
                  # of trials is about 100 for point_dim = 5 
SAMPLES = 10000    # number of points with MND PDF
RADIUS = 0.03     # an initial radius of neighborhood of a maximum
CM_BUFFER_SIZE = 10 # number of top-N points with largest rewards to calculate a covariance matrix 
...
try:
    ...
    while True:
        ...
        if CM:
            ...
            # if cm_idling < 10:
            #     if sorted (list (ix)) == list (range (CM_BUFFER_SIZE)): cm_idling += 1
            #     else: cm_idling = 0

            radius = RADIUS/300.
#            radius = RADIUS/300. if cm_idling < 10 else RADIUS/3000.
#            radius = RADIUS/30. if cm_idling < 10 else RADIUS/300.
